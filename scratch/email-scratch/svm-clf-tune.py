...
model_base = make_pipeline(preprocessing.QuantileTransformer(), svm.SVC()) #LogisticRegression()

# Set the parameters by cross-validation
tuning_parameters = [{'svc__kernel': ['rbf'], 'svc__gamma': [1e-3, 5e-3, 1e-4],
...
def main():
    ...
    model_tuner = GridSearchCV(model_base, tuning_parameters, cv=2, n_jobs=-1, verbose=2)# GridSearchCV(model_base, tuning_parameters, cv=ten_fold_cv, n_jobs=-1, verbose=2)

    #joblib.load('grid_search_svm_full.pkl') #
